<template>
	<div>
		<div class="container" :style='{"minHeight":"100vh","backgroundAttachment":"fixed","alignItems":"flex-start","background":"url(http://codegen.caihongy.cn/20220806/074c8f66072b42f38321bb70ac6564f6.gif)","display":"flex","width":"100%","backgroundSize":"cover","backgroundPosition":"center center","backgroundRepeat":"no-repeat","justifyContent":"center"}'>
			<el-form v-if="pageFlag=='register'" :style='{"border":"10px solid #fff","padding":"40px 0 20px","boxShadow":"0 0px 0px rgba(64, 158, 255, .8)","margin":"0","borderRadius":"0 0 80px 80px","background":"rgba(255,255,255,.5)","width":"1400px","height":"auto"}' ref="rgsForm" class="rgs-form" :model="rgsForm">
				<div v-if="true" :style='{"margin":"0 0 10px 0","color":"#000","textAlign":"center","width":"100%","lineHeight":"80px","fontSize":"28px","fontWeight":"bold"}' class="title">网红酒店预定系统的设计与实现注册</div>
				<el-form-item :style='{"width":"40%","padding":"0","margin":"0 auto 20px","height":"auto"}' class="list-item" v-if="tableName=='kehu'">
				...
				<button :style='{"border":"0","cursor":"pointer","padding":"0 10px","boxShadow":"0 0 0px rgba(64, 158, 255, .5)","margin":"20px auto 5px","color":"#fff","display":"block","outline":"none","borderRadius":"0","background":"#FE8D51","width":"40%","fontSize":"18px","fontWeight":"bold","height":"70px"}' type="button" class="r-btn" @click="login()">注册</button>
	...
	methods: {
		...
		login() {
			var url=this.tableName+"/register";
					if((!this.ruleForm.zhanghao) && `kehu` == this.tableName){
					...
					if((!this.ruleForm.xingming) && `kehu` == this.tableName){
					...
					if(`kehu` == this.tableName && this.ruleForm.nianling&&(!this.$validate.isIntNumer(this.ruleForm.nianling))){
					...
					if(`kehu` == this.tableName && this.ruleForm.shouji&&(!this.$validate.isMobile(this.ruleForm.shouji))){
					...
					if(`kehu` == this.tableName && this.ruleForm.shenfenzheng&&(!this.$validate.checkIdCard(this.ruleForm.shenfenzheng))){
...
<style lang="scss" scoped>
	.container {
	  position: relative;
	  background: url(http://codegen.caihongy.cn/20220806/074c8f66072b42f38321bb70ac6564f6.gif);
